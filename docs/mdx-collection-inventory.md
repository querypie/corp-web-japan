...
| Use cases | 29 | List: `/demo/use-cases`<br>Detail: `/use-cases/:id/:slug`<br>ID redirect: `/use-cases/:id` | `src/content/use-cases/*.mdx` | `public/use-cases/<id>/...` | `src/lib/publications/use-cases/records.ts`<br>`src/lib/publications/use-cases/get-post.ts` | Standard publication family. Note that list and detail route prefixes intentionally differ. |
