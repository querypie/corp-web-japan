...
- Next.js 문서에서는 [`page`](https://nextjs.org/docs/app/api-reference/file-conventions/page)를 ["route에 고유한 UI"](https://nextjs.org/docs/app/api-reference/file-conventions/page)를 정의하는 파일로 설명합니다.
- Feature-Sliced Design 문서에서는 [`Pages`](https://feature-sliced.design/docs/reference/layers#pages)를 웹사이트나 앱을 구성하는 단위이며, [`screens`나 `activities`](https://feature-sliced.design/docs/reference/layers#pages)라고도 부른다고 설명합니다.
- React 문서의 일반적인 표현으로는 [component hierarchy](https://react.dev/learn/thinking-in-react#step-1-break-the-ui-into-a-component-hierarchy)의 top-level component 또는 parent component 쪽에 더 가깝습니다.
...
- 작은 button, card, heading wrapper 같은 것들은 UI primitive나 [Atomic Design의 atoms / molecules](https://atomicdesign.bradfrost.com/chapter-2/) 쪽에 가깝습니다.
- hero block, feature block, pricing table block처럼 비교적 큰 UI 묶음은 [section component 또는 widget](https://feature-sliced.design/docs/reference/layers#widgets)에 가깝습니다.
- business logic 없이 렌더링 책임에 집중하는 경우는 [presentational component](https://www.patterns.dev/react/presentational-container-pattern/)라는 표현이 잘 맞습니다.
...
특히 Next.js의 [`layout.tsx`](https://nextjs.org/docs/app/api-reference/file-conventions/layout)는 framework 용어로서의 layout이고, 이 문서에서 말하는 `page.tsx`는 그 layout 안쪽에 들어가는 page-level leaf composition에 더 가깝습니다.
