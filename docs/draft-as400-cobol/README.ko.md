...
| `as400-technical-overview.ko.md` | AS/400의 CPU, OS, 브랜드 변화와 현재 기술 현황 정리 |
| `market-estimate.ko.md` | 일본 IBM i/RPG/COBOL 레거시 시장 고객 수와 시장 규모 추정 보고서 |
...
1. 정적 서비스 페이지의 초기 공개 경로를 `/services/as400-cobol`로 두고, 필요 시 더 설명적인 `/services/as400-cobol-migration` 후보를 비교
2. `AS400 / COBOL Migration`을 QueryPie 제공 서비스로 명시하는 전제에서 상세 서비스 범위 확인
3. AI 분석, 설계서 생성, 테스트 케이스 생성, Java/API/Cloud 전환을 충분히 공개하는 전제에서 메시지 정리
