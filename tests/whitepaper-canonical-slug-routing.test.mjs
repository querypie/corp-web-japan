...
test("whitepaper detail routes implement id-only canonical redirects", () => {
  ...
  const content = readSource("src/content/publications/whitepaper.ts");
  ...
  assert.equal(existsSync(new URL("../src/content/whitepaper/28.mdx", import.meta.url)), true);
  ...
  assert.match(content, /const WHITEPAPER_POSTS_ROOT = path\.join\(process\.cwd\(\), "src\/content\/whitepaper"\)/);
...
test("whitepaper list page is now driven by MDX-derived whitepaper items", () => {
  ...
  assert.match(whitepapersPage, /import \{ whitepaperItems \} from "@\/content\/publications\/whitepaper"/);
  assert.match(whitepapersPage, /items=\{whitepaperItems\}/);
  assert.match(resources, /export \{ whitepaperItems \} from "@\/content\/publications\/whitepaper"/);
  assert.doesNotMatch(resources, /publications\/whitepapers/);
});
