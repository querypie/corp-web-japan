...
test("InternalEventsDemoHeroToggle renders two rectangular selectable buttons and syncs hero mode through the URL query", () => {
  const file = "src/components/sections/events/internal-events-demo-hero-toggle.tsx";
  const source = readSource(file);

  assert.equal(sourceExists("src/components/sections/events/internal-events-demo-hero-toggle.tsx"), true);
