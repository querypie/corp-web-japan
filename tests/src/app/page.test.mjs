...
test("top page remains readable whether static marketing content is externalized or route-local", () => {
  ...
  assert.match(topPage, /const topPageFinalDemoUrl =\n\s+"https:\/\/querypie\.ai\/contact-us\?inquiry=ai-consulting&product=aip";/);
  assert.match(topPage, /<SolutionChoiceTitle>社内業務を効率化したい<\/SolutionChoiceTitle>/);
  assert.match(topPage, /<SolutionChoiceTitle>自社サービスをAI化したい<\/SolutionChoiceTitle>/);
