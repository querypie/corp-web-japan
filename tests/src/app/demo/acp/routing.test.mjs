...
test("ACP demo public list page and canonical routes are driven by ACP demo MDX publication records", () => {
  ...
  const loader = readSource("src/lib/publications/demo/acp/get-post.ts");
  const records = readSource("src/lib/publications/demo/acp/records.ts");
  ...
  assert.equal(sourceExists("src/app\/demo\/acp/page.tsx"), true);
  assert.equal(sourceExists("src/app/t\/demo\/acp/page.tsx"), false);
  assert.equal(sourceExists("src/app\/demo\/acp/[id]/page.tsx"), true);
  assert.equal(sourceExists("src/lib/publications/demo/acp/records.ts"), true);
  assert.equal(sourceExists("src/lib/publications/demo/acp/get-post.ts"), true);
