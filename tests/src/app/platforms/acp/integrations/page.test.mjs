...
test("ACP integrations page exports indexable metadata for the canonical /platforms/acp/integrations route", () => {
  ...
  assert.match(pageSource, /title:\s*"QueryPie: インテグレーション \| QueryPie AI"/);
});

test("ACP integrations public page keeps authored hero copy and CTA in page.tsx", () => {
  assert.match(pageSource, /ACP統合機能/);
  assert.match(pageSource, /データソースに直接接続し、すべてのシステム、アプリケーション、およびサービスを完全に把握することが可能です。/);
  assert.match(pageSource, /from "@\/components\/sections\/acp\/static-page"/);
  assert.match(pageSource, /<AcpPageCta>/);
  assert.match(pageSource, /<AcpPageCtaTitle>まずは小さく、失敗しないAXを始めよう<\/AcpPageCtaTitle>/);
});
...
test("ACP integrations public page keeps category and product catalog route-local with keyword-based filters", () => {
  ...
  assert.ok(labelMatches.length >= 50, `expected category and product label entries, got ${labelMatches.length}`);
  ...
  assert.match(pageSource, /href="\/platforms\/acp\/integrations\?category=all"/);
  assert.match(pageSource, /href=\{`\/platforms\/acp\/integrations\?category=\$\{category\.key\}`\}/);
