...
test("/platforms/aip keeps route-local copy/composition while the layout primitives live in the AIP section module", () => {
  ...
  assert.match(routeSource, /<AipHeroMockup posterSrc=/);
  assert.doesNotMatch(routeSource, /QueryPie%20AIP%20-%20Secure%20Enterprise%20Agentic%20AI%20Platform\.mp4/);
  ...
  assert.match(sectionSource, /export function AipHeroCopy[\s\S]*flex w-full flex-col items-start gap-5 text-left/);
  assert.match(sectionSource, /export function AipHeroTitle[\s\S]*text-\[36px\][\s\S]*sm:text-\[40px\][\s\S]*lg:text-\[44px\]/);
  assert.match(sectionSource, /export function AipHeroLead[\s\S]*max-w-\[420px\][\s\S]*text-\[16px\][\s\S]*leading-\[26px\]/);
  assert.match(sectionSource, /export function AipHeroMockup/);
  assert.match(sectionSource, /<Image src=\{posterSrc\} alt="QueryPie AIPの画面イメージ"/);
  assert.doesNotMatch(sectionSource, /<video/);
  assert.doesNotMatch(sectionSource, /autoPlay/);
