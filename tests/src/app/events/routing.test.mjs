...
test("event canonical list and detail routes are driven by event MDX publication records", () => {
  ...
  assert.equal(sourceExists("src/app/t/events/page.tsx"), false);
  assert.equal(sourceExists("src/app/events/[id]/page.tsx"), true);
  assert.equal(sourceExists("src/app/features/demo/webinars/[id]/route.ts"), true);
  assert.equal(sourceExists("src/app/features/demo/webinars/[id]/[slug]/route.ts"), true);
  assert.equal(sourceExists("src/lib/publications/events/records.ts"), true);
  assert.equal(sourceExists("src/lib/publications/events/get-post.ts"), true);
