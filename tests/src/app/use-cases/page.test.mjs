...
test("/use-cases uses the dedicated demo sidebar with the active use-case label", () => {
  const source = readSource("src/app/use-cases/page.tsx");
  ...
  assert.match(source, /canonical: "\/use-cases"/);
});

test("/use-cases opts into the shared load-more flow with URL-restored visible counts", () => {
  const source = readSource("src/app/use-cases/page.tsx");
