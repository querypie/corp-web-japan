...
test("launch-risk CTA targets resolve to explicit anchors or real destinations", () => {
  ...
  assert.match(homeContent, /const demoUseCasesUrl = "\/demo\/use-cases"/);
  assert.match(homeContent, /floatingCta: \{ label: "業務に合うAI活用を相談する", href: "\/contact-us\?product=ai-crew" \}/);
  assert.match(homeContent, /secondaryCta: \{ label: "業務に合うAI活用を相談する", href: "\/contact-us\?inquiry=ai-consulting&product=ai-crew" \}/);
  assert.match(homeContent, /primaryCta: \{ label: "進め方を相談する", href: "\/contact-us\?inquiry=ai-consulting&product=ai-crew" \}/);
  ...
  assert.match(topPageContent, /primaryCta: \{ label: "お問い合わせ", href: "\/contact-us\?inquiry=ai-consulting" \}/);
  assert.match(topPageContent, /secondaryCta: \{[\s\S]*label: "資料をダウンロード",[\s\S]*href: "https:\/\/www\.querypie\.com\/ja\/features\/documentation\/aip-introduction-download",?[\s\S]*\}/);
  ...
  assert.match(aiCrewPage, /<FloatingConversionCta href="\/contact-us\?product=ai-crew" \/>/);
  ...
  assert.match(aiDashiPage, /const aiDashiFloatingUrl = "\/contact-us\?product=ai-dashi"/);
  ...
  assert.match(aiDashiFaq, /href="\/contact-us\?inquiry=ai-consulting&product=ai-dashi"/);
