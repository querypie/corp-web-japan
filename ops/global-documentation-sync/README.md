...
Durable production evidence는 public repo의 sanitized ledger로 남깁니다.
운영 issue는 `querypie/corp-web-japan#688`입니다. Local raw report는 host에만
남고 `mM:7d` retention 후 삭제됩니다. Spot CVM이 회수되면 local report는
사라질 수 있으므로, recovery boundary는 public sanitized ledger comment와
remote branch/Draft PR입니다.
...
`EVIDENCE_ISSUE_NUMBER=688`과 `DURABLE_EVIDENCE_REQUIRED=1`을 함께 설정하면
success, `no_candidate`, failure 모두 durable evidence publish가 terminal gate가
됩니다. Publish가 실패하면 service도 failed로 끝나며, Draft PR은 delete/merge
하지 않습니다.
...
Public sanitized ledger는 남지만 raw local report는 7일 retention과 Spot reclamation
boundary를 넘지 않습니다.
...
report path context 뒤에 reason section을 `expand: false`로 보내 Slack이 `See more`로
접을 수 있게 합니다. Reason은 redaction/sanitization 뒤 2400자 이하로 제한합니다.
