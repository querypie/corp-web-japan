...
export const listPrivacyPolicySlugs = cache(async function listPrivacyPolicySlugs() {
...
export const getLatestPrivacyPolicySlug = cache(async function getLatestPrivacyPolicySlug() {
  const slugs = await listPrivacyPolicySlugs();
  return slugs[0] ?? null;
});

export const hasPrivacyPolicySlug = cache(async function hasPrivacyPolicySlug(slug: string) {
  const slugs = await listPrivacyPolicySlugs();
