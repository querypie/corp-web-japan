# System Access Controller render comparison notes
...
- Stage URL: `https://stage.querypie.ai/t/platforms/acp/system-access-controller`
- Live URL: `https://www.querypie.com/ja/solutions/acp/system-access-controller`
- Local route source: `src/app/t/platforms/acp/system-access-controller/page.tsx`
...
| --- | --- | ---: | ---: | --- | --- |
| desktop | `16px` / `16px` | `9410` / `10546` | `11` / `24` | `52px` / `52px` | `32px` / `32px` |
| mobile | `16px` / `14px` | `10683` / `9630` | `11` / `24` | `36px` / `32px` | `26px` / `20px` |
...
Evidence:
- Desktop scroll height is `9410` on stage and `10546` on live.
- Mobile scroll height is `10683` on stage and `9630` on live.
- Stage renders `11` major `main section/article` nodes on desktop, while live exposes `24`. The live count includes more nested production section wrappers, but the visual page also contains a production FAQ block that stage does not render.
...
- `src/app/t/platforms/acp/system-access-controller/page.tsx` for route-local section composition.
...
Evidence:
- Live desktop includes the FAQ block around document top `8388` with questions about SaaS service behavior, authentication handling, security standards, and compatibility with existing security solutions.
...
Likely source area to inspect next:
- Add route-local FAQ content in `src/app/t/platforms/acp/system-access-controller/page.tsx` or a narrow shared ACP FAQ section if all four child pages intentionally share the same production FAQ contract.
...
- However, the measured section top/height sequence diverges after the first intro blocks. For example on desktop, stage reaches the final CTA at document top `8420` while live reaches the final CTA at document top `9265` after also rendering the FAQ block.
...
The stage page is content-complete enough to show the core SAC story, but it is not render-identical to the live production page. The most actionable differences are:
