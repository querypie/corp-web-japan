# System Access Controller 렌더링 비교 메모
...
- 로컬 route source: `src/app/t/platforms/acp/system-access-controller/page.tsx`
...
근거:
- Desktop scroll height는 stage `9410`, live `10546`입니다.
- Mobile scroll height는 stage `10683`, live `9630`입니다.
...
다음에 확인할 source area:
- route-local section composition: `src/app/t/platforms/acp/system-access-controller/page.tsx`
...
근거:
- Live desktop에는 document top 약 `8388` 위치에 FAQ block이 있습니다. 질문은 SaaS service 여부, authentication 처리 방식, security standards, 기존 security solutions와의 compatibility에 관한 내용입니다.
...
- page별 문구 소유권을 route-local로 유지해야 한다면 `src/app/t/platforms/acp/system-access-controller/page.tsx`에 FAQ content를 직접 두는 방향을 검토합니다.
...
- 하지만 초반 intro block 이후 measured section top/height sequence가 달라집니다. 예를 들어 desktop에서 stage final CTA는 document top 약 `8420`에 도달하고, live final CTA는 FAQ block까지 포함한 뒤 document top 약 `9265`에 도달합니다.
...
Stage page는 핵심 SAC story를 보여줄 만큼 content는 갖추고 있지만, live production page와 render-identical한 상태는 아닙니다. 가장 actionable한 차이는 다음입니다.
