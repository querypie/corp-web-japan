...
export async function generateMetadata({ params }: UseCaseDetailPageProps): Promise<Metadata> {
  ...
  const canonicalUrl = absoluteUrl(getUseCasePublicationHref(id, record.slug), await getRequestDeployedSiteUrl());
...
export default async function UseCaseDetailPage({ params }: UseCaseDetailPageProps) {
  ...
  return (
    <main {...componentNameDebugProps("UseCasePostPage")} className="relative overflow-x-hidden bg-white text-slate-950">
      <SiteHeader />
