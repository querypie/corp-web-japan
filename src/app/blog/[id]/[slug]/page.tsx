...
export async function generateMetadata({ params }: BlogDetailPageProps): Promise<Metadata> {
  ...
  const canonicalUrl = absoluteUrl(getBlogPublicationHref(id, record.slug), await getRequestDeployedSiteUrl());
...
export default async function BlogDetailPage({ params }: BlogDetailPageProps) {
  ...
  return (
    <main {...componentNameDebugProps("BlogPostPage")} className="relative overflow-x-hidden bg-white text-slate-950">
      <SiteHeader />
