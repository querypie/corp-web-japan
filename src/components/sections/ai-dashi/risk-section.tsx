...
export function AIDashiRiskLead({ children }: { children: ReactNode }) {
  return <p className="text-[14px] font-semibold leading-7 tracking-[0.02em] text-[#8c4a2f] sm:text-[15px]">{children}</p>;
}

export function AIDashiRiskTitle({ children }: { children: ReactNode }) {
  return <h2 className="mt-2 text-[28px] font-bold leading-[1.18] tracking-[-0.04em] text-slate-950 sm:text-[34px] lg:text-[38px] lg:whitespace-nowrap">{children}</h2>;
}

export function AIDashiRiskBody({ children }: { children: ReactNode }) {
  return <div className="mt-6 space-y-5 text-[15px] leading-8 text-slate-600 [&_strong]:font-semibold [&_strong]:text-slate-800">{children}</div>;
}
