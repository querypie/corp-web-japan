...
export function AIDashiWallCardsBody({ children }: { children: ReactNode }) {
  return <p className="mx-auto mt-5 max-w-[710px] text-left text-base leading-7 text-slate-500">{children}</p>;
}
...
export function AIDashiWallCardsCard({ children }: { children: ReactNode }) {
  return (
    <article className="flex h-full flex-col rounded-[1.8rem] border border-black/6 bg-white p-6 shadow-[0_22px_56px_-44px_rgba(15,23,42,0.14)] transition duration-300 hover:-translate-y-1 hover:shadow-md">
      {children}
    </article>
  );
}
...
export function AIDashiWallCardsCardIcon({ children }: { children: ReactNode }) {
  return <div className="flex h-12 w-12 items-center justify-center rounded-[12px] bg-[#eef1f4] text-[#15181d]">{children}</div>;
}
