...
export function AICrewHeroShell({ children }: { children: ReactNode }) {
  return (
    <div {...componentNameDebugProps("AICrewHeroShell")} className="relative z-10 mx-auto overflow-hidden rounded-[2.2rem] border border-[#D6E2F6] bg-[linear-gradient(135deg,#FBFDFF_0%,#EEF4FF_46%,#F8FBFF_100%)] px-6 py-8 shadow-[0_28px_90px_-56px_rgba(15,42,95,0.22)] sm:px-8 lg:px-[30px] lg:py-10">
...
export function AICrewHeroContent({ children }: { children: ReactNode }) {
  return <div {...componentNameDebugProps("AICrewHeroContent")} className="max-w-[620px] lg:pl-4">{children}</div>;
}

export function AICrewHeroEyebrow({ children }: { children: ReactNode }) {
  return (
    <RevealOnScroll {...componentNameDebugProps("AICrewHeroEyebrow")}>
...
export function AICrewHeroTitle({ children }: { children: ReactNode }) {
  return (
    <RevealOnScroll {...componentNameDebugProps("AICrewHeroTitle")} delayMs={40}>
...
export function AICrewHeroTitleLine({
  ...
  return (
    <span {...componentNameDebugProps("AICrewHeroTitleLine")}
...
export function AICrewHeroSubcopy({ children }: { children: ReactNode }) {
  return (
    <RevealOnScroll {...componentNameDebugProps("AICrewHeroSubcopy")} delayMs={70}>
...
export function AICrewHeroBody({ children }: { children: ReactNode }) {
  return (
    <RevealOnScroll {...componentNameDebugProps("AICrewHeroBody")} delayMs={90}>
...
export function AICrewHeroActions({ children }: { children: ReactNode }) {
  return (
    <RevealOnScroll {...componentNameDebugProps("AICrewHeroActions")} delayMs={140}>
...
export function AICrewHeroPrimaryAction({ href, children }: { href: string; children: ReactNode }) {
  return (
    <Link {...componentNameDebugProps("AICrewHeroPrimaryAction")} href={href} className={aiCrewPrimaryButtonClass}>
      {children}
    </Link>
  );
}
...
export function AICrewHeroSecondaryAction({ href, children }: { href: string; children: ReactNode }) {
  return (
    <Link {...componentNameDebugProps("AICrewHeroSecondaryAction")} href={href} className={aiCrewSecondaryButtonClass}>
      {children}
    </Link>
  );
}
...
export function AICrewHeroVisual({ src, alt }: { src: string; alt: string }) {
  return (
    <RevealOnScroll {...componentNameDebugProps("AICrewHeroVisual")} variant="scale" delayMs={160}>
