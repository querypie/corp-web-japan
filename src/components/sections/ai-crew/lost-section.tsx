...
export function AICrewLostProblemContent({ children }: { children: ReactNode }) {
  return <div className="max-w-[880px] border-l-4 border-[#2563EB] pl-5">{children}</div>;
}

export function AICrewLostProblemTitle({ children }: { children: ReactNode }) {
  return <h2 className="text-[28px] font-bold leading-[1.18] tracking-[-0.04em] text-slate-950 sm:text-[34px] lg:text-[38px]">{children}</h2>;
}

export function AICrewLostProblemBody({ children }: { children: ReactNode }) {
  return <div className="mt-6 space-y-5 text-[15px] leading-8 text-slate-600">{children}</div>;
}
...
export function AICrewLostWhitepaperContent({ children }: { children: ReactNode }) {
  return <div className="max-w-[630px]">{children}</div>;
}

export function AICrewLostWhitepaperEyebrow({ children }: { children: ReactNode }) {
  return <div className="inline-flex rounded-full border border-[#C9D8F5] bg-white/90 px-4 py-2 text-[12px] font-semibold tracking-[0.12em] text-[#163A7A] shadow-[0_14px_36px_-24px_rgba(15,42,95,0.28)]">{children}</div>;
}

export function AICrewLostWhitepaperTitle({ children }: { children: ReactNode }) {
  return <h3 className="mt-4 text-[24px] font-semibold leading-[1.35] tracking-[-0.03em] text-white sm:text-[28px]">{children}</h3>;
}

export function AICrewLostWhitepaperBody({ children }: { children: ReactNode }) {
  return <p className="mt-3 text-[14px] leading-7 text-white/82">{children}</p>;
}
