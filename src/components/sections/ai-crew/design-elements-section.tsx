...
export function AICrewDesignElementsGrid({ children }: { children: ReactNode }) {
  return <div className="mt-10 grid items-stretch gap-3 md:grid-cols-2 xl:grid-cols-5">{children}</div>;
}
...
export function AICrewDesignElementHeading({ children }: { children: ReactNode }) {
  return <h3 className="text-[18px] font-semibold leading-[1.45] tracking-[-0.02em] text-[#101828]">{children}</h3>;
}

export function AICrewDesignElementBody({ children }: { children: ReactNode }) {
  return <div className="mt-2 text-[14px] leading-6 text-[#667085]">{children}</div>;
}
