...
export function Button({
  ...
  const buttonClassName = cn(
    ...
    !isText && [
      ...
      size === "md" &&
        "min-h-11 px-5 py-2.5 body-md md:px-[24px] md:py-[12px]",
      size === "sm" &&
        "h-8 px-3 py-1.5 body-sm md:h-9 md:px-4 md:py-2",
    ],

    // variant별 색상/배경 (토큰 직접 사용)
    variant === "primary" && "bg-[var(--brand)] text-[var(--white)] hover:bg-[#f2743d]",
    variant === "dark" && "bg-[var(--black)] text-[var(--white)] hover:bg-[#333333]",
    variant === "outline" && "bg-[var(--white)] text-[var(--fg)] hover:bg-[var(--hover)]",
    isOutline && "border border-[var(--border)]",
    variant === "text" && "inline-flex items-center gap-[4px] body-sm text-[var(--fg)] transition-opacity duration-200 hover:opacity-70 group",
