...
function buildPrompt(role, candidate, sourceHtml, targetMdx, correctionFindings) {
  ...
  const scope = role === "fidelity"
    ...
    : role === "japanese-editorial"
      ? "Review only Japanese reader-facing text in targetMdx. Apply every loaded Japanese editorial pass, including frontmatter copy, natural syntax, register, repetition, terminology, and notation. Calibrate severity strictly: isolated wording described as merely やや, 少し, or locally awkward is a note; minor requires a repeated or clearly actionable quality defect; major requires a defect that materially prevents publication or pervades the document. Do not review immutable source evidence, source URLs, target route architecture, or filesystem paths."
