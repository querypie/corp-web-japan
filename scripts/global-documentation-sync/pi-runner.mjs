...
function buildPrompt(role, candidate, sourceHtml, targetMdx, correctionFindings) {
  ...
  if (role === "writer") {
    return [
      ...
      "targetFiles must list candidate.targetMdxPath, candidate.heroImagePath, and every candidate.assets[].targetPath exactly once; it may contain nothing else. Convert YouTube iframe watch/youtu.be URLs to https://www.youtube.com/embed/<video-id>.",
      `DATA=${data}`,
    ].join("\n");
  }
  const scope = role === "fidelity"
    ? "Check every omission, addition, meaning or certainty change, name, date, number, heading, figure, caption, link, and CTA. Judge semantic and communicative equivalence, not literal wording. Accept natural Japanese editorial rewrites of awkward source copy when claims, tone, emphasis, and function remain intact; never demand exact punctuation or mechanically restore translationese."
    : role === "japanese-editorial"
      ? "Review only Japanese reader-facing text in targetMdx. Apply every loaded Japanese editorial pass, including frontmatter copy, natural syntax, register, repetition, terminology, and notation. Calibrate severity strictly: isolated wording described as merely やや, 少し, or locally awkward is minor; major requires a defect that materially prevents publication or pervades the document. Do not review immutable source evidence, source URLs, target route architecture, or filesystem paths."
