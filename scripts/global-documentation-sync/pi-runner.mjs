...
function buildPrompt(role, candidate, sourceHtml, targetMdx, correctionFindings) {
  ...
  if (role === "writer") {
    const familyContract = candidate.targetFamily === "news"
      ? "News contract: frontmatter must not contain author. When candidate.meta.dateIso is present, frontmatter date must equal candidate.meta.dateIso exactly as the News listing/publication date. If sourceHtml or an outlink body shows a different dateline, preserve that dateline in reader-facing body copy when fidelity requires, but never replace the News frontmatter date with it. News sourceLabel must equal candidate.resolvedSourceLabel exactly. News redirectUrl must equal candidate.resolvedRedirectUrl exactly when non-null and must be omitted when candidate.resolvedRedirectUrl is null. News relatedIds must match candidate.meta.relatedIds exactly; never invent or leave unresolved related IDs."
      : "";
    return [
      ...
      correctionFindings?.length
        ? `CORRECTION MODE: Start from targetMdx, not from a fresh rewrite. Resolve all ${correctionFindings.length} supplied actionable findings one by one in this response, including every minor-or-higher item. Apply each concrete suggestion by default; deviate only when it would change source meaning. Preserve every unaffected line and never reintroduce an earlier finding.`
        : "INITIAL GENERATION MODE: create the publication from the authoritative source.",
      "Use the loaded publication skills. Preserve semantic meaning, tone, emphasis, and all inventories while producing natural Japanese; do not mechanically preserve or restore awkward source wording. candidate.sourceLocale is authoritative: use only candidate.meta.title[candidate.sourceLocale], candidate.meta.summary[candidate.sourceLocale], and sourceHtml for reader-facing language; never substitute another locale because of the slug, filename, image text, or other metadata. Prioritize fidelity to source meaning over any conflicting editorial suggestion. Never apply an editorial suggestion that weakens, omits, or hedges a source claim, certainty, or emphasis. Preserve exact source-authoritative proper-name and brand spelling wherever it appears in candidate.meta.title[candidate.sourceLocale], candidate.meta.summary[candidate.sourceLocale], or sourceHtml; do not transliterate, localize, legal-name-normalize, or change script unless that exact form already appears in those source-authoritative fields. Japanese output must contain no Korean code points unless an intentional proper-noun exception is recorded. Use candidate.heroImagePublicPath for frontmatter heroImageSrc. Frontmatter id must be a quoted YAML string equal to String(candidate.targetId), for example `id: \"19\"`; never emit an unquoted or numeric id. Frontmatter slug must equal candidate.meta.id. When candidate.meta.hideHeroImage is true and the family supports it, set hideHeroImageOnDetail: true; never emit hideHeroImage. Set author exactly to candidate.resolvedAuthor when non-null and omit author when null. When correctionFindings is non-empty, reconcile findings by preserving meaning and improving Japanese, rather than blindly copying a review suggestion; resolve every supplied actionable finding, including minor findings, without unrelated rewrites; preserve already-correct text from targetMdx.",
      familyContract,
  ...
  const scope = role === "fidelity"
    ? `Check every omission, addition, meaning or certainty change, name, date, number, heading, figure, caption, link, and CTA. Perform a complete review on every attempt and report all current drift you can find in the current draft. Do not defer obvious issues to later attempts. Judge semantic and communicative equivalence, not literal wording. Accept natural Japanese editorial rewrites of awkward source copy when claims, tone, emphasis, and function remain intact; never demand exact punctuation or mechanically restore translationese. Generic marketing hyperbole such as “all/everything you need to know” may be localized as 「全体像」「要点」 without meaning drift when the concrete body scope remains complete; do not force unnatural literal emphasis. Correcting an objectively mistranslated technical term to its established Japanese meaning (for example PAM = 特権アクセス管理) is not drift when the acronym and product scope remain unchanged.${candidate.targetFamily === "news" ? " For News, candidate.meta.dateIso and the frontmatter date are contract-owned listing/publication metadata. Do not demand that the News frontmatter date match an outlink or source-body dateline. Review source/body dateline fidelity separately in the reader-facing copy and report body-dateline drift there when needed." : ""}`
    : role === "japanese-editorial"
      ? "Review only Japanese reader-facing text in targetMdx. Apply every loaded Japanese editorial pass, including frontmatter copy, natural syntax, register, repetition, terminology, and notation. Preserve all source claims, certainty, and emphasis. The review must not propose semantic weakening or omission. Do not emit findings for merely preferable alternative wording when the current Japanese is acceptable. Classify any reader-facing unnatural, stilted, or translationese wording that has a concrete correction as actionable minor or higher; reserve note only for non-actionable observations that require no text change. Calibrate severity strictly: isolated wording described as merely やや or 少し awkward is still minor when a concrete correction exists; major requires a defect that materially prevents publication or pervades the document. Preserve exact source-authoritative proper-name and brand spelling from candidate.meta.title[candidate.sourceLocale], candidate.meta.summary[candidate.sourceLocale], and sourceHtml. Do not suggest transliteration, localization, legal-name normalization, or script changes unless that exact alternative form already appears in those source-authoritative fields. Do not review immutable source evidence, source URLs, target route architecture, or filesystem paths."
      : `Check family frontmatter field presence/types, author, related IDs, candidate.targetRoute, local asset/link paths, PNG Open Graph image, gating, downloads, and MDX contract. Do not judge title, description, or body wording against the source; fidelity and Japanese reviewers exclusively own copy quality. candidate.production.canonicalUrl is immutable Global source evidence, not the Japan target route; never require it to use the Japan id-based route. A non-null candidate.heroImagePath is deterministic proof that the CLI generated that PNG even though it is separate from candidate.assets. ${candidate.targetFamily === "news" ? "For News, frontmatter date must equal candidate.meta.dateIso exactly when present; this listing/publication metadata is contract-owned. Distinguish that field from any outlink or source-body dateline, which belongs only to reader-facing body fidelity review." : ""} Report only actual contract violations; never emit findings or notes for valid omitted optional fields such as author on use-cases.`;
  return [
