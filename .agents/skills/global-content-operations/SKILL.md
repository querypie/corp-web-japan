...
10. Attach `report.operationsSummary` before building the local Slack preview. Render one concise change section: new Global count, existing Japan match, natural-language verdict/action, tracking deltas, and final counts. In operations mode, omit historical `Ignored` item details and show details only for unresolved `Untracked` items. Show the user this Block Kit preview, the full review table, unresolved items, and final counts.
